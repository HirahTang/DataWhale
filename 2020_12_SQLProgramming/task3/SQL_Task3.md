...
《_sql**基础**教程**第2版_》用一句话非常凝练的概括了视图与表的区别—“是否保存了实际的数据”。所以视图并不是数据库真实存储的数据表，它可以看作是一个窗口，通过这个窗口我们可以看到数据库表中真实存在的数据。所以我们要区别视图和数据表的本质，即视图是基于真实表的一张虚拟的表，其数据来源均建立在真实表的基础上。

[![YnTlq5vR65wRlPdk](http://datawhale.club/uploads/default/optimized/1X/43b68fb04b76905b2554b3d7bfce78dc795348fb_2_690x382.png)](http://datawhale.club/uploads/default/original/1X/43b68fb04b76905b2554b3d7bfce78dc795348fb.png "YnTlq5vR65wRlPdk")

YnTlq5vR65wRlPdk1050×582 189 KB
...
[![lpBdy6SjY7FwbpsC](http://datawhale.club/uploads/default/optimized/1X/87f3e18e1b9a9edeb20cfa02a01d118b905763a3_2_599x500.png)](http://datawhale.club/uploads/default/original/1X/87f3e18e1b9a9edeb20cfa02a01d118b905763a3.png "lpBdy6SjY7FwbpsC")

lpBdy6SjY7FwbpsC1016×848 130 KB
...
_在 MySQL中视图的定义是允许使用 ORDER BY 语句的，但是若从特定视图进行选择，而该视图使用了自己的 ORDER BY 语句，则视图定义中的 ORDER BY 将被忽略。_
...
![7fifOiOMfOgfqrOp](http://datawhale.club/uploads/default/original/1X/31e2e7e533e8e5d8a10a0aee34f93d9b9d918916.png)
...
![Y0Jf0hzxQOXCQsUp](http://datawhale.club/uploads/default/original/1X/aba3b1da871ee65cc75d101152a96354a1f030e6.png)
...
![U7c1MZRuoUuHyFcj](http://datawhale.club/uploads/default/original/1X/133477c379e38c8dc98cbc67d922d413d0246b67.png)
...
![9fCOadE9zuEZS9WI](http://datawhale.club/uploads/default/original/1X/c01ad9eed89c137b99897cbd4ac9f157d36fd28d.png)

此时观察原表也可以发现数据也被更新了

[![5wmjMqRztHrCGiKe](http://datawhale.club/uploads/default/optimized/1X/59f1f89b0593839292197135ce2e6c00950224d5_2_690x136.png)](http://datawhale.club/uploads/default/original/1X/59f1f89b0593839292197135ce2e6c00950224d5.png "5wmjMqRztHrCGiKe")

5wmjMqRztHrCGiKe1318×260 36.4 KB
...
![sRdkXBxMgxT8mQYj](http://datawhale.club/uploads/default/original/1X/8d39bfcc1908251a1f1b1fcbecee6828e8a823a7.png)
...
但是如果刚接触的话一定会比较疑惑关联查询的执行过程，这里有一个[博客  14](https://zhuanlan.zhihu.com/p/41844742)讲的比较清楚。在这里我们简要的概括为：

1.  首先执行不带WHERE的主查询
2.  根据主查询结果匹配product_type，获取子查询结果
3.  将子查询结果再与主查询结合执行完整的SQL语句

_在子查询中像标量子查询，嵌套子查询或者关联子查询可以看作是子查询的一种操作方式即可。_
...
[![rlYYZNMWCnUbbMI4](http://datawhale.club/uploads/default/original/1X/764db4d8ab153bec0425b1407732335b3b1b9bbe.png)](http://datawhale.club/uploads/default/original/1X/764db4d8ab153bec0425b1407732335b3b1b9bbe.png "rlYYZNMWCnUbbMI4")

rlYYZNMWCnUbbMI4801×534 23.2 KB
